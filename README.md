...
- زر **↓ كل الأقسام TXT + JSON** ينزّل ZIP يشمل التقرير كاملًا، وليس نتائج الفلتر فقط.
- السيرفرات: `server.txt` + `server.json`؛ البروتوكولات: `protocol.txt` + `protocol.json`، وبالمثل باقي الأقسام.
- فحص المشروع يحفظ `sections/` تلقائيًا بجانب `report.json` داخل مجلد تشغيل الفحص، مع `index.json` ودليل قراءة.
- CLI مع `-o report.json` ينشئ `report.sections/` تلقائيًا؛ `--sections-dir NEWDIR` يحدد مكانًا آخر. لا تُستبدل مجلدات التصدير السابقة.
- الأقسام الفارغة محفوظة أيضًا، والنصوص UTF-8. ملفات JSON تحتفظ بالأدلة ومواقعها والثقة والتحذيرات دون اقتصار على الأعمدة المرئية.
- هذه إعادة تنظيم **للنتائج المتاحة** وليست استرجاعًا إضافيًا للمصدر؛ مخرجات المحركات الأصلية تظل في مجلداتها. [دليل التصدير والأسماء](docs/section-exports.md).
